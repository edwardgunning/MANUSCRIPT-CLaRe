source("code/rebuttal-simulation-helpers.R")
...
# aside... for reprting:
####
# mu alpha and sigma_alpha were actually mean and varianc
# on exponentiated scale:
...
pdf("figures/skew-simulation-boxplot.pdf", width = 5, height = 5)
par(mfrow = c(1, 1))
boxplot(cbind(p_vals_qd_1, p_vals_qd_2),
        names = c(paste0("CLaRe, Power = ", 100 * mean(p_vals_qd_1<0.05), "%"),
...
pdf("figures/skew-normal-data-generation.pdf", width = 8, height = 8)
par(cex = 1.25)
...
matplot(Q_2, type = "l", ylim = c(range(Q_1, Q_2)), col = scales::alpha(4, 0.5), lty = 1,
        xlab = "p", ylab = "Q(p)", main = "Skew-normal Group")

boxplot(cbind(apply(Q_1, 2, mean),
              apply(Q_2, 2, mean)),
        col = c("red4", 4),
        names = c("N", "SN"))
title("Means")
boxplot(log(cbind(apply(Q_1, 2, var),
                  apply(Q_2, 2, var))),
        col = c("red4", 4),
        names = c("N", "SN"))
title("Log of Variances")
boxplot(cbind(apply(Q_1, 2, e1071::skewness),
