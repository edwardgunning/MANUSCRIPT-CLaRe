...
for (i in seq_along(sample_sizes)) {
  ...
  pca_list[[i]] <- GLaRe(mat = mat_i, kf = sample_sizes[i], verbose = FALSE)
  dwt_list[[i]] <- GLaRe(mat = array_i, learn = "dwt.2d", latent_dim_to = 810, kf = sample_sizes[i], verbose = FALSE)
}


saveRDS(object = list(dwt = dwt_list, pca = pca_list),
        file = "data/sample-size-results-03.rds")

results <- readRDS(file = "data/sample-size-results-03.rds")
...
legend_cex <- 0.7
cairo_pdf(file = "figures/eye-sample-size-results-results-03.pdf", width = 15, height = 15 / 2, family = "DejaVu Sans")
