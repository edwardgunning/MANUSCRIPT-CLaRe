...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/additional-plots-01.pdf}
    \caption{Reconstructions of $8$ observations from the \texttt{phenome} dataset (dotted lines) overlaid on the original observations (solid lines). The reconstructions were computed from the final model fit of PCA at the qualifying dimension $qd=X$. The figure was generated using the \texttt{plot\_1D\_reconstruction()} function from the \pkg{GLaRe} package.}
