...
\add{In principle, GLaRe can be applied to any type of complex object dataset whose observations can be stored in a vector, matrix or array. 
In this section, we introduce three motivating object datasets that can be viewed as functions on one or two-dimensional Euclidean or spherical domains (e.g., curves and images).
}
...
\begin{itemize}
    \item Put in different complexities of learning for different representations (phase transition)/ possibly add discussion (Figure \ref{fig:continuum-sketch}).
\end{itemize}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figures/continuum-sketch.pdf}
    \caption{A continuum that quantifies the degree to which a latent feature representation relies on the data. Methods towards the left assume more structure for the transformation and learn less of it from the data, whereas methods towards the right assume less structure and learn more from the data.}
    \label{fig:continuum-sketch}
\end{figure}
...
\add{Achieving a chosen tolerance level for every observation (i.e., requiring that even the worst case meets the near-losslessness threshold) can sometimes be unrealistic in practice.
...
In this case, using the worst case is overly stringent and results in a latent feature representation that is higher-dimensional and more complex than required for the majority of features.
To rectify this, we generalise the ideas the notion of near lossslessness of the entire dataset, and require it for is met for \emph{quantiles} or \emph{percentiles} of the distribution of individual generalisation errors, where the worst case observation is the $100$th percentile. 
...
That is, if we evaluate the information loss of a latent feature representation method on the same dataset that was used to learn that representation, then our information loss estimates will tend to be overly optimistic.
As such, it is necessary to use an independent ``validation" dataset to assess the information loss of a learned latent feature representation method.
Generally, with limited data, it is inefficient to perform a single split of the dataset into training and validation sets, as a single random split will tend to be variable (i.e., if the random split were performed again, the new validation set would give a different estimate of information loss) \parencite{harrell_jr_regression_2015}.
Additionally, because we are interested in the individual values of information loss (rather than an average), sample splitting would only provide us with these values for observations included in the test set. 
Instead, we employ \emph{cross-validation} \parencite{stone_cross-validatory_1974}, where the data are divided systematically into different training and validation splits, called folds, and the training and validation is performed separately for each split.
When we are interested in an average measure, cross-validation will tend to be more stable than sample splitting because the estimates are averaged over many different folds.
In our case, it additionally produces an our-of-sample estimate of information loss for each individual observation in our dataset.
