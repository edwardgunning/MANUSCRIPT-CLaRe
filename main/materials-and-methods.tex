\section{Methodology}\label{sec:materials-and-methods}
...
Often, a small number of observations (e.g., such as the one traced by the blue points and line in Figure \ref{fig:ind-losses}) possess idiosyncratic features that cannot be captured by a representation that is otherwise compact and near-lossless for the \jsm{vast} majority of the observations.
In this case, using the worst case is overly stringent and results in a latent feature representation that is higher-dimensional and more complex than required for the \jsm{vast} majority of observations.
Therefore, we generalise the \jsm{notion of near losslessness for} the entire dataset, and require it for is met for \emph{quantiles} or \emph{percentiles} of the distribution of individual generalization errors, \jsm{in which} the worst case observation is the $100$th percentile. 
